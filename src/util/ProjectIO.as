...
public class ProjectIO {
	...
	public function ProjectIO(app:mBlockRT):void {
		this.app = app;
	}
	...
	private function decodeFromZipFile(zipData:ByteArray):ScratchObj {
		...
		if(jsonData.indexOf("PicoBoard")>-1){
			...
			if(!mBlockRT.app.extensionManager.checkExtensionSelected("Makeblock")){
				mBlockRT.app.extensionManager.onSelectExtension("Makeblock");
			}
		}else if(jsonData.indexOf("Arduino.")>-1){
			if(!mBlockRT.app.extensionManager.checkExtensionSelected("Arduino")){
				mBlockRT.app.extensionManager.onSelectExtension("Arduino");
			}
		}

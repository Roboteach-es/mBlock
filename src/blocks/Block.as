...
//		mBlockRT.app.runtime.checkForGraphicEffects();
	}
...
/*
	public function showHelp():void {
		...
		if((i = op.indexOf('.')) > -1) {
			...
			if(mBlockRT.app.extensionManager.isInternal(extName))
				mBlockRT.app.showTip('ext:'+extName);
			else
				DialogBox.notify('Help Missing', 'There is no documentation available for experimental extension "'+extName+'".', mBlockRT.app.stage);
		}
		else {
			mBlockRT.app.showTip(op);
		}
	}
*/
	public function duplicateStack(deltaX:Number, deltaY:Number):void {
		if (isProcDef()) return; // don't duplicate procedure definition
		var forStage:Boolean = mBlockRT.app.viewedObj() && mBlockRT.app.viewedObj().isStage;
		var newStack:Block = BlockIO.stringToStack(BlockIO.stackToString(this), forStage);
		newStack.x = x + deltaX;
		newStack.y = y + deltaY;
		parent.addChild(newStack);
		mBlockRT.app.gh.grabOnMouseUp(newStack);
	}
...
//		var top:Block = topBlock();
//		// TODO: Remove any waiting reporter data in the mBlockRT.app.extensionManager
//		if (parent is Block) Block(parent).removeBlock(this);
...
//		if (top != this) x += top.width + 5;
//		removeComments();
//		mBlockRT.app.runtime.recordForUndelete(this, x, y, 0, mBlockRT.app.viewedObj());
//		mBlockRT.app.scriptsPane.saveScripts();
//		mBlockRT.app.runtime.checkForGraphicEffects();
//		return false;
	}
	private function removeComments():void{
		var comments:Array = attachedCommentsIn(mBlockRT.app.scriptsPane);
		...
		mBlockRT.app.scriptsPane.fixCommentLayout();
	}
	...
	public function objToGrab(evt:MouseEvent):Block {
		if (isEmbeddedParameter() || isInPalette()) return duplicate(false, mBlockRT.app.viewedObj() is ScratchStage);
		return this;
	}
	...
	public function click(evt:MouseEvent):void {
		...
		if(topBlock().op.indexOf("runArduino")>-1){
			if(!mBlockRT.app.stageIsArduino){
				mBlockRT.app.changeToArduinoMode();
			}
			return;
		}
		mBlockRT.app.runtime.interp.toggleThread(topBlock(), mBlockRT.app.viewedObj(), 1);
	}

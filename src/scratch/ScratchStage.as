...
public class ScratchStage extends ScratchObj {
	...
	public function spritesAndClonesNamed(spriteName:String):Array {
		...
		var app:mBlockRT = parent as mBlockRT;
		if (app != null) {
...
//		if(mBlockRT.app.isIn3D) return mBlockRT.app.render3D.getUIContainer();
		return this;
	}

	override protected function updateImage():void {
		super.updateImage();
//		if(mBlockRT.app.isIn3D)
//			mBlockRT.app.render3D.getUIContainer().transform.matrix = transform.matrix.clone();
...
//			if(mBlockRT.app.isIn3D) mBlockRT.app.render3D.updateRender(videoImage);
		}
...
//			mBlockRT.app.render3D.updateRender(penLayer);
...
//		if(!mBlockRT.app.render3D || s.width < 1 || s.height < 1) return null;
		return null;
/*
		...
		var bmd:BitmapData = mBlockRT.app.render3D.getRenderedChild(s, b.width*s.scaleX, b.height*s.scaleY, for_carry);
	...
	public function setVideoState(newState:String):void {
		...
		mBlockRT.app.libraryPart.showVideoButton();
	...
	public function clearPenStrokes():void {
		...
		penActivity = false;
//		if(mBlockRT.app.isIn3D) mBlockRT.app.render3D.updateRender(penLayer);
	}
...
//		if(mBlockRT.app.isIn3D) mBlockRT.app.render3D.updateFilters(spr, effects);
	}
	...
	public function getBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		...
		if(mBlockRT.app.isIn3D) {
			var b:Rectangle = s.currentCostume().bitmap ? s.img.getChildAt(0).getBounds(s) : s.getVisibleBounds(s);
			bm1 = mBlockRT.app.render3D.getOtherRenderedChildren(s, 1);
			//mask = 0x80F8F8F0;
		}
		else {*/
			// OLD code here
...
//		if(mBlockRT.app.isIn3D) mBlockRT.app.render3D.updateRender(dispObj, renderID, renderOpts);
	}
...
//		if(mBlockRT.app.isIn3D) mBlockRT.app.render3D.getRender(bm);
//		else
		bm.draw(this);
	...
	public function updateInfo():void {
		...
		if (mBlockRT.app.projectID != '') info.projectID = mBlockRT.app.projectID;
		info.videoOn = isVideoOn();
		info.swfVersion = mBlockRT.versionString;

		delete info.loadInProgress;
		if (mBlockRT.app.loadInProgress) info.loadInProgress = true; // log flag for debugging

		if (this == mBlockRT.app.stagePane) {
			// If this is the active stage pane, record the current extensions.
			var extensionsToSave:Array = mBlockRT.app.extensionManager.extensionsToSave();
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (!(obj is ScratchSprite) || mBlockRT.app.editMode) mBlockRT.app.setSaveNeeded();
			return true;
		}
		mBlockRT.app.setSaveNeeded();
		if ((obj is MediaInfo) && obj.fromBackpack) {
			...
			var app:mBlockRT = root as mBlockRT;
			// Add sprites
			if (obj.mysprite) {

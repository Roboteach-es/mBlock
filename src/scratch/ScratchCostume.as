...
public class ScratchCostume {
	...
	public function setBitmapData(bm:BitmapData, centerX:int, centerY:int):void {
		...
		rotationCenterX = centerX;
		rotationCenterY = centerY;
		if (mBlockRT.app && mBlockRT.app.viewedObj() && (mBlockRT.app.viewedObj().currentCostume() == this)) {
			mBlockRT.app.viewedObj().updateCostume();
			mBlockRT.app.refreshImageTab(true);
		}
	}
	...
	public function setSVGData(data:ByteArray, computeCenter:Boolean, fromEditor:Boolean = true):void {
		...
		function refreshAfterImagesLoaded():void {
			...
			if (mBlockRT.app && mBlockRT.app.viewedObj() && (mBlockRT.app.viewedObj().currentCostume() == thisC)) {
				mBlockRT.app.viewedObj().updateCostume();
				mBlockRT.app.refreshImageTab(fromEditor);
			}
			svgLoading = false;
		}
...
//trace('found bounds: '+new Rectangle(minX, minY, maxX - minX, maxY - minY));
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		var oldQuality:String = mBlockRT.app.stage.quality;
		mBlockRT.app.stage.quality = StageQuality.LOW;
		bm.draw(dispObj, m);
		mBlockRT.app.stage.quality = oldQuality;

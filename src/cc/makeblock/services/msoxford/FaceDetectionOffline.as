package cc.makeblock.services.msoxford
{
	...
	public class FaceDetectionOffline
	{
		...
		public function start():void{
			...
			initUI();
			initDetector();
//			mBlockRT.app.track("/OxfordAi/realtime-face/launch");
		}
		public function stop():void{
			trace("stop");
			mBlockRT.app.stageObj().removeEventListener(Event.RENDER,cameraReadyHandler);
//			mBlockRT.app.track("/OxfordAi/realtime-face/close");
		}
		private function initUI():void{
			mBlockRT.app.stageObj().removeEventListener(Event.RENDER,cameraReadyHandler);
			mBlockRT.app.stageObj().addEventListener(Event.RENDER,cameraReadyHandler);
		...
		{
			if(mBlockRT.app.stageObj().videoImage){
				detectionMap.draw(mBlockRT.app.stageObj().videoImage.bitmapData,drawMatrix,null,"normal",null,true);
		...
		private function detectionHandler( e :ObjectDetectorEvent ):void
		{
			if( e.rects ){
				...
				if(results.length>0){
					mBlockRT.app.extensionManager.extensionByName("Microsoft Cognitive Services").stateVars["realFaceResultReceived"] = results;
					mBlockRT.app.runtime.realFaceResultReceived.notify(true);
				}
			}
		}
	}

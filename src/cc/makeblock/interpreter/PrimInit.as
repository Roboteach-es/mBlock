package cc.makeblock.interpreter
{
	...
	internal class PrimInit
	{
		...
		{
			if(!mBlockRT.app.interp.turboMode){
...
//			if (target.activeThread.firstTime) {
			...
			mBlockRT.app.runtime.allStacksAndOwnersDo(findReceivers);
			var threadList:Array = [];
			for each(var item:Array in receivers){
				var newThread:Thread = mBlockRT.app.interp.toggleThread(item[0], item[1]);

...
package cc.makeblock.interpreter {
	...
	public function addPrimsTo(provider:FunctionProvider):void {
		...
		provider.register('answer', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.lastAnswer) });

		provider.register('mousePressed', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.gh.mouseIsDown) });
		provider.register('mouseX', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.stagePane.scratchMouseX()) });
		provider.register('mouseY', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.stagePane.scratchMouseY()) });
		provider.register('timer', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.timer()) });
		provider.register('timerReset', function(thread:Thread, argList:Array):void { mBlockRT.app.runtime.timerReset() });
		...
		provider.register('soundLevel', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.soundLevel()) });
		provider.register('isLoud', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.isLoud() )});
		provider.register('timestamp', primTimestamp);
		provider.register('timeAndDate', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.getTimeString(argList[0])) });

		// sensor
		provider.register('sensor:', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.getSensor(argList[0])) });
		provider.register('sensorPressed:', function(thread:Thread, argList:Array):void { thread.push( mBlockRT.app.runtime.getBooleanSensor(argList[0])) });
...
//		if(true || !app.isIn3D) {
			...
			for each (s2 in mBlockRT.app.stagePane.spritesAndClonesNamed(arg))
...
//		testSpr.graphics.drawRect(0, 0, stageBM.width, stageBM.height);
	...
	private function showBM(bm:BitmapData):void {
		if (debugView == null) {
			...
			debugView.x = 100;
			debugView.y = 600;
			mBlockRT.app.addChild(debugView);
		}
...
//		testBM.bitmapData = srcBM;
	...
	private function stageBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		return mBlockRT.app.stagePane.getBitmapWithoutSpriteFilteredByColor(s, c);
	}

	private function primAsk(thread:Thread, argList:Array):void {
		if (mBlockRT.app.runtime.askPromptShowing()) {
			thread.suspend();
...
//		if (interp.activeThread.firstTime) {
			...
			if ((obj is ScratchSprite) && (obj.visible)) {
				...
				mBlockRT.app.runtime.showAskPrompt('');
			} else {
				mBlockRT.app.runtime.showAskPrompt(question);
			}
			thread.suspend();
			mBlockRT.app.runtime.askPromptHideSignal.add(thread.resume, true);
...
//		}
	...
	private function primKeyPressed(thread:Thread, argList:Array):void {
		...
		if (key == 'space') ch = 32;
		thread.push( mBlockRT.app.runtime.keyIsDown[ch]);
	}
...
//		var s:ScratchSprite = thread.userData;
		...
		var obj:ScratchObj = mBlockRT.app.stagePane.objNamed(argList[1]);
	...
	private function mouseOrSpritePosition(arg:String):Point {
		if (arg == '_mouse_') {
			var w:ScratchStage = mBlockRT.app.stagePane;
			return new Point(w.scratchMouseX(), w.scratchMouseY());
		} else {
			var s:ScratchSprite = mBlockRT.app.stagePane.spriteNamed(arg);
	...
	private function primShowWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) mBlockRT.app.runtime.showVarOrListFor(argList[0], false, obj);
	}

	private function primHideWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) mBlockRT.app.runtime.hideVarOrListFor(argList[0], false, obj);
	}

	private function primShowListWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) mBlockRT.app.runtime.showVarOrListFor(argList[0], true, obj);
	}

	private function primHideListWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) mBlockRT.app.runtime.hideVarOrListFor(argList[0], true, obj);
	}

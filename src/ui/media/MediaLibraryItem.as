...
//		}
		...
		if (fileType(md5) == 'svg'){
			var data:ByteArray = mBlockRT.app.server.getAsset(md5);
			if (data) {
				importer = new SVGImporter(XML(data));
				importer.loadAllImages(svgImagesLoaded);
			}
			...
			mBlockRT.app.server.getThumbnail(md5, thumbnailWidth, thumbnailHeight, setThumbnail);
		}
	}
	...
	private function setSpriteThumbnail(done:Function):void {
		...
		if (spriteCache[spriteMD5]) {
			...
			data = mBlockRT.app.server.getAsset(spriteMD5).toString();
		}
	...
	private function downloadAndPlay():void {
		...
		var wavData:ByteArray = mBlockRT.app.server.getAsset(dbObj.md5);
		if (!wavData) return;
		sndData = wavData;
		startPlayingSound();
	}

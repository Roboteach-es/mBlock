...
public class MediaInfo extends Sprite {
	...
	public function objToGrab(evt:MouseEvent):* {
		var result:MediaInfo = mBlockRT.app.createMediaInfo({
			type: objType,
			name: objName,
			md5: md5
		});
		if (mycostume) result = mBlockRT.app.createMediaInfo(mycostume, owner);
		if (mysound) result = mBlockRT.app.createMediaInfo(mysound, owner);
		if (mysprite) result = mBlockRT.app.createMediaInfo(mysprite);
		if (scripts) result = mBlockRT.app.createMediaInfo(scripts);
	...
	protected function duplicateMe():void {
		if (owner) {
			if (mycostume) mBlockRT.app.addCostume(mycostume.duplicate());
			if (mysound) mBlockRT.app.addSound(mysound.duplicate());
		}
	}

	protected function deleteMe(ignore:* = null):void {
		if (owner) {
			mBlockRT.app.runtime.recordForUndelete(this, 0, 0, 0, owner);
			if (mycostume) {
				owner.deleteCostume(mycostume);
				mBlockRT.app.refreshImageTab(false);
			}
			if (mysound) {
				owner.deleteSound(mysound);
				mBlockRT.app.refreshSoundTab()
			}
		}
	}

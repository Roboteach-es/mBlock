...
public class MediaPane extends ScrollFrameContents {
	...
	public function MediaPane(app:mBlockRT, type:String):void {
		this.app = app;
		isSound = (type == 'sounds');
		refresh();
	}
	...
	private function costumeItems():Array {
		...
		for each (var c:ScratchCostume in viewedObj.costumes) {
			result.push(mBlockRT.app.createMediaInfo(c, viewedObj));
		}
		return result;
	}
	...
	private function soundItems():Array {
		...
		for each (var snd:ScratchSound in viewedObj.sounds) {
			result.push(mBlockRT.app.createMediaInfo(snd, viewedObj));
		}
		return result;
	}

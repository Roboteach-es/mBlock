...
//			runtime.interp.redraw();
	...
	private function specForCmd():String {
		...
		if(i > -1) {
			var spec:Array = mBlockRT.app.extensionManager.specForCmd(cmd);
	...
	private function getValue(runtime:ScratchRuntime):* {
		...
		if(index >= 0){
			...
			var ext:ScratchExtension = mBlockRT.app.extensionManager.extensionByName(extName);
...
/*
		if(cmd.indexOf('.') > -1) {
			...
			if(spec) {
				...
				return mBlockRT.app.interp.evalCmd(block);
			}
		}
*/
		return "unknown: " + cmd;
	...
	public function doubleClick(evt:MouseEvent):void {
		if (!mBlockRT.app.editMode) return;
	...
	public function menu(evt:MouseEvent):Menu {
		function handleMenu(item:int):void {
			...
			if (item == 10) {
				visible = false;
				mBlockRT.app.updatePalette(false);
			}
		}
		if (!mBlockRT.app.editMode) return null;
...
//			m.addItem("text", 4);
	...
	private function mouseDown(evt:MouseEvent):void {
		...
		if (p.y > 20) mBlockRT(root).gh.setDragClient(this, evt);
	}

...
public class ListWatcher extends Sprite {
	...
	private function importLines(lines:Array):void {
		...
		DialogBox.ask(
			...
			'1', mBlockRT.app.stage, gotColumn);
	}
...
//		mBlockRT.app.interp.redraw();
	...
	private function addItem(b:IconButton = null):void {
		...
		if ((root is mBlockRT) && !(root as mBlockRT).editMode) return;
...
//		if (limitedCloudView &&
//			mBlockRT.app.isLoggedIn() && mBlockRT.app.editMode &&
//			(mBlockRT.app.projectOwner == mBlockRT.app.userName)) {
//				limitedCloudView = false; // only project owner can view cloud list contents
//		}
		var isEditable:Boolean = mBlockRT.app.editMode && !limitedView;
		updateElementCount();
		removeAllCells();
		visibleCells = [];
		visibleCellNums = [];
